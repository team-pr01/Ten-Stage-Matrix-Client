...
const RecentAddition = () => {
  ...
  return (
    <div className="font-Outfit rounded-[15px] border-[3px] border-neutral-25/20 bg-neutral-30 flex flex-col py-7 px-[34px] w-full md:w-[80%] h-full md:h-[300px] overflow-y-auto custom-scrollbar">
          ...
          <tbody>
            {referralList?.map((transaction, index) => (
              <tr key={index} className="border-b border-neutral-110">
                ...
                {/* Level */}
                <td className="py-3">{transaction?.label}</td>

                {/* Joined */}
                <td className="py-3">{transaction?.joined}</td>
              </tr>
            ))}
          </tbody>
        </table>
